  ...
  class SpecificContentsController < ApplicationController
    ...
    def create
      ...
      respond_to do |format|
        if @specific_content.save
          format.html { redirect_to admin_specific_content_path(@specific_content.id), notice: 'Le contenue spécifique a été créé.' }
    ...
    def update
      respond_to do |format|
        if @specific_content.update(specific_content_params)
          format.html { redirect_to admin_specific_content_path(@specific_content.id), notice: 'Les modifications ont été enregistrées.' }
    ...
    private
      ...
      def specific_content_params
        params.require(:specific_content).permit(:title, :subtitle, :content, :external_link, :specific_content_picture)
      end
  end
end
